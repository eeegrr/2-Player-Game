...
class GameGUI:
    ...
    def create_widgets(self):
        self.master.title("Split numbers")
        ...
        self.importsubgame.add_command(
            label="AlphaBeta against person",
            command=partial(
                self.restart_with_computer,
        ...
        self.importsubgame.add_command(
            label="Person against AlphaBeta",
            command=partial(
                self.restart_with_computer,
        ...
        self.menuBar.add_cascade(menu=self.filemenu, label="Play")
        self.menuBar.add_cascade(menu=self.actionmenu, label="Settings")
    ...
    def update_ui(self):
        ...
        if not self.game.done:
            info = f"""Points:
                1. player: {self.game.points[0]}
                2. player: {self.game.points[1]}

                Turn: {self.game.player+1}. player"""
        else:
            info = f"""Game ended with result:
                1. player: {self.game.points[0]}
                2. player: {self.game.points[1]}
